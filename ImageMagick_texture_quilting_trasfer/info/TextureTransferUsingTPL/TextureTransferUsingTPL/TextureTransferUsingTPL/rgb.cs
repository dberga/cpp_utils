...
namespace TextureTransfer
{
    ...
    public struct rgb
    {
        ...
        public static unsafe rgb[,] FromBitmap(Bitmap bmp)
        {
            ...
            if (bpp < 3)
                throw new ArgumentException("FromBitmap: invalid argument exception");
        ...
        {            
            return ToBitmap(A, new Rectangle(0,0,A.GetLength(1),A.GetLength(0)));
        }
        ...
        public static unsafe Bitmap ToBitmap(rgb[,] A, Rectangle rect)
        {
            ...
            BitmapData bmpData = bmp.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly, fmt);
            byte* ptr = (byte*)(void*)bmpData.Scan0;
            int stride = bmpData.Stride;
            int offset = stride - bpp * width;
            for (int y = rect.Y; y < rect.Y + height; y++, ptr += offset)
