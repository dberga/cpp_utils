...
float colorkernel[3][3];
		...
		colorkernel[0][0]=COLOR(x--,y--);
		colorkernel[0][1]=COLOR(x--,y);
		colorkernel[0][2]=COLOR(x--,y++);
		colorkernel[1][0]=COLOR(x,y--);
		colorkernel[1][1]=COLOR(x,y);
		colorkernel[1][2]=COLOR(x,y++);
		colorkernel[2][0]=COLOR(x++,y--);
		colorkernel[2][1]=COLOR(x++,y);
		colorkernel[2][2]=COLOR(x++,y++);
		
		//asignamos los vecinos del pixel de mascara
		float v1 = MASCARA(maskposX--,maskposY--);
		float v2 = MASCARA(maskposX--,maskposY);
		float v3 = MASCARA(maskposX--,maskposY++);
		float v4 = MASCARA(maskposX,maskposY--);
		float v6 = MASCARA(maskposX,maskposY++);
		float v7 = MASCARA(maskposX++,maskposY--);
		float v8 = MASCARA(maskposX++,maskposY);
		float v9 = MASCARA(maskposX++,maskposY++);
...
//miramos que tipo de pixel es (esquina, borde...) y hacemos el cuadrado de las diferencias entre vecinos, y despues sumamos los valores de semejanza
			...
			float unR = pow(COLOR(maskposX--,maskposY--) - colorkernel[0][0],2);
			float dosR = pow(COLOR(maskposX--,maskposY) - colorkernel[0][1],2);
			float tresR = pow(COLOR(maskposX--,maskposY++) - colorkernel[0][2],2);
			float quatreR = pow(COLOR(maskposX,maskposY--)-colorkernel[1][0],2);
			float setR = pow(COLOR(maskposX++,maskposY--) - colorkernel[2][0],2);
			...
			float dosR = pow(COLOR(maskposX--,maskposY) - colorkernel[0][1],2);
			float tresR = pow(COLOR(maskposX--,maskposY++) - colorkernel[0][2],2);
			float sisR = pow(COLOR(maskposX,maskposY++) - colorkernel[1][2],2);
			float nouR = pow(COLOR(maskposX++,maskposY++)- colorkernel[2][2],2);
			...
			float quatreR = pow(COLOR(maskposX,maskposY--)-colorkernel[1][0],2);
			float setR = pow(COLOR(maskposX++,maskposY--) - colorkernel[2][0],2);
			float vuitR = pow(COLOR(maskposX++,maskposY) - colorkernel[2][1],2);
			float nouR = pow(COLOR(maskposX++,maskposY++)- colorkernel[2][2],2);
...
/****************************************** ENCONTRAMOS MATRIZ MASCARA *******************************************/
  ...
  for (int i=0; i<fil; i++){
	for(int j=0; j<col; j++){
		...
		if((maskR(i,j)>223.0) && (maskG(i,j)>223.0) && (maskB(i,j)>223.0)){
