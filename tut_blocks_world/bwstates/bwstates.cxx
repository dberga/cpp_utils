...
void get_options(int argc, char *argv[], int *size, int *nstates, long *seed)
{
    ...
    if( *size <= 0 ) {
        fprintf(stderr, "Generator of random blocks world states\n");
        fprintf(stderr, "by John Slaneya and Sylvie Thiebaux\n\n");
        fprintf(stderr, "Usage:\n");
        fprintf(stderr, "\t%s -n <num>\n", argv[0]);
        fprintf(stderr, "\t\twhere <num> is the number of blocks\n", argv[0]);
	exit(2);
    }	
    if (size<0 || *size >SZ) {
...
void print_state_old(state sigma)
{
  int x;
...
void print_state(state sigma)
{
  ...
  for (x=0; x<sigma->N; x++)
    if( 0 == sigma->S[x]+1) 
      printf("block %d on table\n", (x+1));
    else
      printf("block %d on top of block %d\n", (x+1), sigma->S[x]+1);
